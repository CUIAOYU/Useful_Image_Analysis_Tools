...
* **Heatmap Generation**:
    ...
    * **Threshold-Based Heatmap Visualization**:
        ...
        * **Thresholding and Color Mapping Logic**:
            * Pixels with an Aggregate Value **below** the Min Threshold are displayed in **black** (representing low signal or background).
            * Pixels with an Aggregate Value **above** the Max Threshold are displayed in **bright red** (representing saturation or aggregation).
            * Pixels with an Aggregate Value **within** the [Min Value, Max Value] range are colored based on their relative position within this range, transitioning smoothly from **purple** (near Min Value) -> **blue** -> **green** -> **yellow** -> **orange** -> **red** (near Max Value).
...
* **SAM Processing**:
    * **Description**: Combines the Segment Anything Model (AI automatic segmentation) with Aggregate Analysis. It automatically identifies distinct regions in an image, filters these regions based on whether their internal pixels fall within a specified Aggregate Value range, and then performs quantitative analysis and visual marking on the filtered regions.
    ...
    * **Two-Stage Threshold Filtering and Property Calculation**:
        * **Stage 1: Pixel-Level Aggregate Filtering (based on Min/Max Aggregate Threshold)**:
            * For **each** region mask generated by SAM, the program examines all pixels within that region.
            * A region is initially considered to "contain a valid signal" only if it **contains pixels whose Aggregate Values fall within** the user-defined [Min Value, Max Value] range. Pixels outside this range within the region do not contribute to subsequent calculations.
            * If a region contains **no** pixels within this aggregate range, it is entirely ignored.
        * **Stage 2: Region-Level Property Threshold Filtering (based on A/I/R Thresholds)**:
            * For regions that passed Stage 1, the program calculates the following properties **using only the pixels whose Aggregate Values were within the [Min Value, Max Value] range**:
                * **Area (A)**: The total number of these valid pixels.
                * **Intensity (I)**: The sum of the Aggregate Values of these valid pixels.
                * **Ratio (R)**: Intensity / Area.
            * **Final Filtering**: A region is finally marked as a target region only if its calculated A, I, and R values **simultaneously satisfy** the corresponding minimum thresholds set by the user (`A Threshold`, `I Threshold`, `R Threshold` - depending on which metrics the user has chosen to filter by).
    * **Visualization**: Draws outlines (using different colors to distinguish adjacent regions) around the target regions that passed **both filtering stages** on the output image. Optionally annotates these regions with their calculated A/I/R values.
