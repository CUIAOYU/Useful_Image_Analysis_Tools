...
* **对比度增强**:
    * **作用**: 拉伸图像亮暗区域的差异，让图像细节更清楚。
    * **批量**: 可以一次性处理文件夹里所有的图片。
    * **调节**: 你可以自己设置增强的程度（大于1增强，小于1减弱）。
    * **用在哪儿**:
        * 让 X 光片、MRI 里的病灶轮廓更明显。
        * 让显微镜照片里的细胞、组织边缘或内部结构更清楚。
        * 改善扫描的电泳胶图、Western Blot 膜或者老旧实验记录照片的清晰度。

* **聚合分析**:
    * **“聚合度”是啥**: 我定义了一个叫“聚合度”的指标 (0-100)，用来衡量像素有多“暗”。简单说，**颜色越深，聚合度越高**。
    * **怎么算**:
        1.  先把像素的灰度值 (0-255, 0是黑, 255是白) 标准化到 0-1 范围 (`灰度值 / 255.0`)。
        2.  乘以 100。
        3.  用 100 减去上面的结果。
        * **公式**: `聚合度 = 100 - (灰度值 / 255.0) * 100`
        * **例子**: 纯黑像素聚合度是 100，纯白是 0。
    * **可能用途**: 在某些场景下（比如生物染色图片），高聚合度可能意味着“物质密度”更高或“信号”（特指暗信号）更强。
    * **快速评估**: 计算整张图或整个文件夹图片的**平均聚合度**，快速了解样本整体的“暗度”或“聚合水平”。
    * **用在哪儿**:
        * 快速比较不同处理组样本图片整体的染色深浅。
        * 初步筛选大量图片，找出特别暗或特别亮的。
...
* **热力图生成**:
    ...
    * **用在哪儿**:
        * 在组织切片图上，用颜色显示不同区域染色强度或分子表达水平（如果和聚合度相关的话）。
        * 在材料扫描图里，可视化特定密度或成分范围的区域分布。
...
* **SAM 处理**:
    ...
    * **最终计算和标注**:
        * 对于**两步都通过**的区域，程序**只用**第二步找到的“有效像素”来**重新计算**最终的：
            ...
            * **比率 (Ratio)**: 强度 / 面积。
        * 程序会在输出图片上画出这个区域的**原始轮廓**，并用彩色文字标出**最终算出来**的 A/I/R 值。
    ...
    * **用在哪儿**:
        * 自动识别显微镜图片里的所有细胞，计算大小 (Area) 和蛋白表达量 (Intensity)，并且只标记那些表达量达标（聚合度在特定范围）的细胞。
        * 在 IHC 或 IF 图片里，自动分割阳性信号区域（用聚合度阈值定义阳性），并测量面积和总强度。
        * 自动识别活/死细胞染色图片里特定颜色（对应特定聚合度）的细胞，计算数量和面积，忽略其他颜色或背景。

* **批量处理与易用性**:
    * **文件夹处理**: 主要功能都支持一次处理整个文件夹的图片。
    * **图形界面**: 用 PyQt6 做了个直观的界面，选文件、设参数、看日志、打开结果文件夹都挺方便。
