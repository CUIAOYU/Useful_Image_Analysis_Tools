    ...
    def run(self):
        try:
            if self.min_aggregate_threshold >= self.max_aggregate_threshold:
                    raise ValueError("Min aggregate threshold must be lower than Max aggregate threshold")
            ...
            self.log_updated.emit(f"Created output folder: {self.output_folder}")
    ...
    def generate_heatmap(image_path, output_folder, min_aggregate_threshold=0, max_aggregate_threshold=100):
        try:
            ...
            if threshold_range <= 0:
                print(f"Warning: generate_heatmap called with min_aggregate_threshold ({min_aggregate_threshold}) >= max_aggregate_threshold ({max_aggregate_threshold}).")
            ...
            output_path = os.path.join(output_folder, f"{base_name}_output_heatmap.png")
        ...
        try:
            ...
            for idx, filename in enumerate(files):
                ...
                try:
                    ...
                    output_path = os.path.join(self.output_folder, f"{base_name}_output_contrast{ext}")
        ...
        try:
            ...
            for idx, filename in enumerate(files):
                ...
                try:
                    ...
                    method_suffix = []
                    if self.use_area: method_suffix.append("A")
                    if self.use_intensity: method_suffix.append("I")
                    if self.use_area and self.use_intensity: method_suffix.append("R")
                    method_str = "".join(method_suffix) if method_suffix else "processed"

                    output_filename = f"{file_base}_output_{method_str}_agg{self.min_aggregate_threshold}-{self.max_aggregate_threshold}_Athr{self.a_threshold}_Ithr{self.i_threshold:.0f}_Rthr{self.r_threshold:.1f}{file_ext}"
    ...
    def process_image(self, input_path, output_path, mask_generator):
        try:
            ...
            for i, mask_info in enumerate(masks):
                ...
                valid_aggregates = aggregate_values_0_100[
                    (aggregate_values_0_100 >= self.min_aggregate_threshold) &
                    (aggregate_values_0_100 <= self.max_aggregate_threshold)
                ]

                if len(valid_aggregates) == 0:
                    continue

                area = 0
                intensity = 0.0
                ratio = 0.0
                draw_this_region = True

                if self.use_area:
                    area = len(valid_aggregates)
                    if area < self.a_threshold:
                        draw_this_region = False

                if draw_this_region and self.use_intensity:
                    intensity = np.sum(valid_aggregates)
                    if intensity < self.i_threshold:
                        draw_this_region = False

                if draw_this_region and self.use_area and self.use_intensity and area > 0:
                ...
                if draw_this_region:
                    value_text_parts = []
                    if self.use_area:
                        value_text_parts.append(f"A:{area}")
                    if self.use_intensity:
                        if intensity == 0.0 and area > 0:
                             intensity = np.sum(valid_aggregates)
                        value_text_parts.append(f"I:{intensity:.0f}")
                        if self.use_area and area > 0:
                            if ratio == 0.0: ratio = intensity / area
                            value_text_parts.append(f"R:{ratio:.2f}")
                        elif self.use_area:
...
class UsefulImageAnalysisToolIntegration(QMainWindow):
    ...
    def initUI(self):
        ...
        self.open_heatmap_output_btn = QPushButton("Open Heatmap Folder")
        ...
        param_group_proc = QGroupBox("Analysis Parameters & Thresholds")
        param_group_proc.setStyleSheet("QGroupBox { font-weight: bold; }")
        param_layout_proc = QFormLayout(param_group_proc)
        param_layout_proc.setContentsMargins(10, 15, 10, 10)
        param_layout_proc.setVerticalSpacing(10)
        param_layout_proc.setLabelAlignment(Qt.AlignmentFlag.AlignRight)

        # --- UI Order Modification START ---

        self.min_aggregate_spin = QSpinBox()
        self.min_aggregate_spin.setRange(0, 100)
        self.min_aggregate_spin.setValue(35)
        self.min_aggregate_spin.setToolTip("Pixels with Aggregate value below this are ignored.")
        param_layout_proc.addRow("Min Aggregate Threshold:", self.min_aggregate_spin)

        self.max_aggregate_spin = QSpinBox()
        self.max_aggregate_spin.setRange(0, 100)
        self.max_aggregate_spin.setValue(100)
        self.max_aggregate_spin.setToolTip("Pixels with Aggregate value above this are ignored.")
        param_layout_proc.addRow("Max Aggregate Threshold:", self.max_aggregate_spin)

        self.a_threshold_spin = QSpinBox()
        self.a_threshold_spin.setRange(0, 1000000)
        self.a_threshold_spin.setValue(360) # Default 360
        self.a_threshold_spin.setToolTip("Regions with Area (pixel count) less than this value will not be drawn.")
        param_layout_proc.addRow("Min Area (A) Threshold:", self.a_threshold_spin)
        ...
        self.i_threshold_spin.setToolTip("Regions with Intensity (sum of aggregates) less than this value will not be drawn.")
        param_layout_proc.addRow("Min Intensity (I) Threshold:", self.i_threshold_spin)

        self.r_threshold_spin = QDoubleSpinBox()
        self.r_threshold_spin.setRange(0.0, 100.0)
        self.r_threshold_spin.setSingleStep(0.1)
        self.r_threshold_spin.setValue(50.0) # Default 50.0
        self.r_threshold_spin.setDecimals(1)
        self.r_threshold_spin.setToolTip("Regions with Ratio (I/A) less than this value will not be drawn.")
        param_layout_proc.addRow("Min Ratio (R=I/A) Threshold:", self.r_threshold_spin)

        # --- UI Order Modification END ---

        metrics_layout = QHBoxLayout()
        metrics_layout.setSpacing(15)
        # --- MODIFICATION: Update Checkbox Labels ---
        ...
        self.intensity_check = QCheckBox("Intensity (sum of aggregates)")
        ...
        param_layout_proc.addRow("Calculate & Display:", metrics_layout)
        process_scroll_layout.addWidget(param_group_proc)
        ...
        self.box_nms_thresh_spin.setToolTip("IoU threshold for Non-Maximum Suppression (NMS) used to filter duplicate masks")
        ...
        self.crop_n_layers_spin.setToolTip("If >0, the image will be cropped into multiple layers for processing (can use more memory but handle larger images)")
        ...
        self.crop_nms_thresh_spin.setToolTip("IoU threshold for applying NMS between crop regions")
        ...
        self.crop_overlap_ratio_spin.setToolTip("Sets the degree of overlap between crops")
        ...
        self.crop_n_points_downscale_factor_spin.setToolTip("Controls the degree of downsampling of point coordinates during cropping")
        ...
        self.min_mask_region_area_spin.setToolTip("If >0, post-processing will be applied to remove regions smaller than this area (pixels)")
        ...
        self.output_mode_combo.setToolTip(
            ...
            "Note: RLE modes require 'pycocotools' to be installed for processing."
        )
    ...
    def get_processing_output_folder_path(self):
        ...
        method_suffix = []
        if self.area_check.isChecked(): method_suffix.append("A")
        if self.intensity_check.isChecked(): method_suffix.append("I")
        if self.area_check.isChecked() and self.intensity_check.isChecked(): method_suffix.append("R")
        method_str = "".join(method_suffix) if method_suffix else "processed"
        ...
        output_folder_name = f"{input_folder_name}_output_{method_str}_agg{self.min_aggregate_spin.value()}-{self.max_aggregate_spin.value()}_Athr{self.a_threshold_spin.value()}_Ithr{self.i_threshold_spin.value():.0f}_Rthr{self.r_threshold_spin.value():.1f}"
    ...
    def get_heatmap_output_folder_path(self):
        ...
        output_folder_name = f"{input_folder_name}_output_heatmap"
        return os.path.join(parent_dir, output_folder_name)
    ...
    def get_contrast_output_folder_path(self):
        ...
        output_folder_name = f"{input_folder_name}_output_contrast"
        return os.path.join(parent_dir, output_folder_name)
    ...
    def on_calculation_method_changed(self, state):
        if not self.area_check.isChecked() and not self.intensity_check.isChecked():
             ...
             if sender == self.area_check and state == Qt.CheckState.Unchecked.value:
                 self.area_check.setChecked(True)
                 QMessageBox.warning(self, "Selection Required", "At least one calculation method (Area or Intensity) must be selected.")
            ...
            if output_folder:
                if not os.path.exists(output_folder):
                     try:
                         ...
                         QMessageBox.warning(self, "Error", f"Could not create or access output folder:\n{output_folder}\nError: {e}")
                         return
                try:
                ...
                QMessageBox.information(self, "Info", "Output folder path could not be determined (process might need to run first).")
        else:
            ...
            if output_folder:
                if not os.path.exists(output_folder):
                     try: os.makedirs(output_folder)
                         ...
                         QMessageBox.warning(self, "Error", f"Could not create or access heatmap output folder:\n{output_folder}\nError: {e}")
                         return
                try:
                    if sys.platform == "win32": os.startfile(output_folder)
                    elif sys.platform == "darwin": os.system(f"open \"{output_folder}\"")
                    else: os.system(f"xdg-open \"{output_folder}\"")
                except Exception as e:
                    QMessageBox.warning(self, "Error", f"Could not open heatmap output folder:\n{output_folder}\nError: {e}")
            else:
                 QMessageBox.information(self, "Info", "Heatmap output folder path could not be determined.")
        else:
            ...
            if output_folder:
                if not os.path.exists(output_folder):
                     try: os.makedirs(output_folder)
                         ...
                         QMessageBox.warning(self, "Error", f"Could not create or access contrast output folder:\n{output_folder}\nError: {e}")
                         return
                try:
                    if sys.platform == "win32": os.startfile(output_folder)
                    elif sys.platform == "darwin": os.system(f"open \"{output_folder}\"")
                    ...
                    QMessageBox.warning(self, "Error", f"Could not open contrast output folder:\n{output_folder}\nError: {e}")
            else:
                 QMessageBox.information(self, "Info", "Contrast output folder path could not be determined.")
        else:
    ...
    def batch_process_images(self):
        ...
        if not self.area_check.isChecked() and not self.intensity_check.isChecked():
             QMessageBox.warning(self, "Selection Required", "Please select at least one calculation method (Area or Intensity).")
             return
        ...
        self.log_text.append(f"Calculation: Area={'Yes' if use_area else 'No'}, Intensity={'Yes' if use_intensity else 'No'}")
        self.log_text.append(f"Aggregate Range: [{min_agg}-{max_agg}]")
        self.log_text.append(f"Min Area (A) Threshold: {a_threshold}")
        self.log_text.append(f"Min Intensity (I) Threshold: {i_threshold:.0f}")
        self.log_text.append(f"Min Ratio (R=I/A) Threshold: {r_threshold:.1f}")
    ...
    def generate_heatmaps(self):
        ...
        if min_thresh >= max_thresh:
             if not (min_thresh == 100 and max_thresh == 100):
...
if __name__ == "__main__":
    ...
    app.setStyle('Fusion')
    window = UsefulImageAnalysisToolIntegration()
    window.show()
    sys.exit(app.exec())
